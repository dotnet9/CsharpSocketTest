...
##### GeneralProcessItem【0.0.3】

| 字段名     | 数据类型 | 说明                                                         |
| ---------- | -------- | ------------------------------------------------------------ |
| Status     | byte     | 状态：0：正常，1：超时，2：超限，4：切换用户，可以使用位域组合，共8种状态 |
| Data       | byte[3]  | 下面详细说明：GeneralProcessItemData                         |
| UpdateTime | uint     | 更新时间（当天时间戳：当日0点0分0秒计算的时间戳，单位ms）    |

GeneralProcessItemData【0.0.3】

| Offset | Size | 字段            | 说明                                                         |      |
| ------ | ---- | --------------- | ------------------------------------------------------------ | ---- |
| 0      | 10   | GPU             | GPU(所有GPU引擎的最高利用率)，最后一位表示小数位，比如253表示25.3 |      |
| 10     | 1    | GPUEngine       | GPU引擎，0：无，1：GPU 0 - 3D                                |      |
| 11     | 3    | PowerUsage      | 电源使用情况（CPU、磁盘和GPU对功耗的影响），0：非常低，1：低，2：中，3：高，4：非常高 |      |
| 14     | 3    | PowerUsageTrend | 电源使用情况趋势（一段时间内CPU、磁盘和GPU对功耗的影响），0：非常低，1：低，2：中，3：高，4：非常高 |      |
