...
使用`MessagePack`对对象进行二进制压缩，数据包会比常规二进制序列化小。

#### 2.1.2. 网络对象定义【0.0.3】

以下是目前使用到的对象列表：

| 对象名                    | 数据包类型 | 对象Id | 对象版本 | 说明                         |
| ------------------------- | ---------- | ------ | -------- | ---------------------------- |
| RequestBaseInfo           | TCP        | 1      | 1        | 请求服务基本信息             |
| ResponseBaseInfo          | TCP        | 2      | 1        | 响应请求服务基本信息         |
| RequestProcessList        | TCP        | 3      | 1        | 请求进程详细信息列表         |
...
| Data           | byte[8]  | 见ProcessItemData定义 |
| LastUpdateTime | uint     | 上次更新时间          |
| UpdateTime     | uint     | 更新时间              |

##### ProcessItemData【0.0.4】
...
| 58     | 3    | Status          | 进程状态，0：新建状态，1：就绪状态，2：运行状态，3：阻塞状态，4：终止状态 |      |
...
##### UpdateRealtimeProcessList【0.0.3】
...
| Processes | `List<RealtimeProcessItem>?` | 进程列表                                                     |

##### RealtimeProcessItem【0.0.3】

| 字段名 | 数据类型 | 说明         |
| ------ | -------- | ------------ |
| Data   | byte[5]  | 下面详细说明 |

RealtimeProcessItemData【0.0.3】
...
##### UpdateGeneralProcessList【0.0.3】
...
| PageCount | int                         | 总页数                                                       |
| PageIndex | int                         | 页索引，客户端根据收到的进程详细信息列表为基础（有顺序关系），取当前数据包开始进程索引到结束进程索引数据更新更新 |
| Processes | `List<GeneralProcessItem>?` | 进程列表                                                     |

##### GeneralProcessItem【0.0.4】

| 字段名     | 数据类型 | 说明                                                      |
| ---------- | -------- | --------------------------------------------------------- |
| Data       | byte[3]  | 下面详细说明：GeneralProcessItemData                      |
| UpdateTime | uint     | 更新时间（当天时间戳：当日0点0分0秒计算的时间戳，单位ms） |

GeneralProcessItemData【0.0.4】

| Offset | Size | 字段            | 说明                                                         |      |
| ------ | ---- | --------------- | ------------------------------------------------------------ | ---- |
| 0      | 3    | ProcessStatus   | 进程状态，0：新建状态，1：就绪状态，2：运行状态，3：阻塞状态，4：终止状态 |      |
| 3      | 3    | AlarmStatus     | 告警状态：0：正常，1：超时，2：超限，4：切换用户，可以使用位域组合，共8种状态 |      |
| 6      | 10   | GPU             | GPU(所有GPU引擎的最高利用率)，最后一位表示小数位，比如253表示25.3 |      |
| 16     | 1    | GPUEngine       | GPU引擎，0：无，1：GPU 0 - 3D                                |      |
| 17     | 3    | PowerUsage      | 电源使用情况（CPU、磁盘和GPU对功耗的影响），0：非常低，1：低，2：中，3：高，4：非常高 |      |
| 20     | 3    | PowerUsageTrend | 电源使用情况趋势（一段时间内CPU、磁盘和GPU对功耗的影响），0：非常低，1：低，2：中，3：高，4：非常高 |      |
